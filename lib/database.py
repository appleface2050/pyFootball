...
class Mysql(object):
    ...
    def connect(self, dba):
        ...
        self.conn = MySQLdb.connect(host=str(dba['host']),user=str(dba['user']),passwd=str(dba['passwd']),db=str(dba['db']),port=dba['port'])
...
class Model(dict):
    ...
    def truncate_table(self, **kargs):
        dbase = self.get_dbase(**kargs)
        if not self.is_dbase_exist(**kargs):
            raise Exception('%s not exist'%dbase)
        if self.is_table_exist(**kargs):
            self.db.use_dbase(dbase)
            sql = "TRUNCATE TABLE `%s`"%self.get_table(**kargs)
            self.db.execute(sql)
        return True
...
class Query(object):
    ...
    def multi_col_groupby(self,field1,field2):
        self.cache = None
        self.group = 'GROUP BY `%s`,`%s`' % (field1,field2)
        return self
    
    def thrid_col_groupby(self,field1,field2,field3):
        self.cache = None
        self.group = 'GROUP BY `%s`,`%s`,`%s`' % (field1,field2,field3)
        return self
